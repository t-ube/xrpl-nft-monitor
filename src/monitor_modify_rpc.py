#!/usr/bin/env python3
"""
NFTokenModify 監視スクリプト
XRPL ledger RPCを使用して全NFTokenModifyトランザクションを監視し、
変更前後のURIをキャッシュAPIに送信する
5分間隔のcron実行を想定
"""
...
# 設定
MAX_LEDGERS_PER_RUN = 500  # 1回の実行で処理する最大レジャー数
...
def save_modify_history_batch(records: list[dict]) -> None:
    """NFTokenModify履歴をSupabaseにバルクインサート"""
    if not records:
        return
...
def extract_uris_from_modify(tx: dict) -> tuple[str, str | None, str | None] | None:
    """
    NFTokenModifyトランザクションのmetaから対象NFTokenIDと変更前後のURIを抽出
    Returns: (nftoken_id, previous_uri, current_uri) or None
    """
    ...
    return nftoken_id, previous_uri, current_uri
...
def process_transactions(
    ...
    """
    取得済みトランザクションからNFTokenModifyを処理する
    ...
    for tx in transactions:
        # NFTokenModify かつ成功したトランザクションのみ
        if tx.get("TransactionType") != "NFTokenModify":
            continue
        ...
        try:
            result = extract_uris_from_modify(tx)
            if not result:
            ...
            # 変更前URIをキャッシュ対象に追加
            uri_skipped = False
            if previous_uri and is_cacheable_uri(previous_uri):
                pending_uris.append(previous_uri)
                print(f"    Previous URI -> queued")
            elif previous_uri:
                uri_skipped = True

            # 変更後URIをキャッシュ対象に追加
            if current_uri and is_cacheable_uri(current_uri):
                pending_uris.append(current_uri)
                print(f"    Current URI  -> queued")
            elif current_uri:
                uri_skipped = True
...
def main():
        ...
        for idx in batch:
            ...
            try:
                ...
                if ledgers_since_checkpoint >= checkpoint_interval:
                    ...
                    # Supabaseバルクインサート
                    if pending_records:
                        save_modify_history_batch(pending_records)
    ...
    print(f"=== Done: {total_processed} modifies ({total_skipped} skipped), {total_errors} errors, {elapsed:.1f}s ===")
